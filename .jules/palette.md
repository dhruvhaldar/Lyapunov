...
## 2026-04-09 - Focus loss during async state changes
**Learning:** When inputs are temporarily disabled during asynchronous operations (like "Loading..." states after an "onChange" or "onSelect"), the browser drops keyboard focus from that element. If the user was navigating via keyboard or screen reader, their focus is reset to the document body, severely disrupting the accessibility flow.
**Action:** Store the active element (e.g. `const hadFocus = document.activeElement === element`) *before* applying the `disabled` state. In the `finally` block of the Promise, check `hadFocus` and restore focus to the input (`element.focus()`) after re-enabling it.
...
## 2026-09-22 - Extracting Secondary UI Elements from Transient Buttons
**Learning:** When dynamically updating a button's `innerHTML` to display transient states (e.g., swapping to a success icon), secondary UI elements nested inside the button (like keyboard shortcut hints via `<kbd>`) are destroyed. This breaks CSS transitions and leads to visual flickering or missing elements upon restoration.
**Action:** Extract secondary UI elements as sibling elements within a wrapper `span` (e.g., `<span class="icon-btn-container"> <button>...</button> <kbd>...</kbd> </span>`) to prevent transient `innerHTML` replacement from destroying the hint. Use CSS general sibling combinators (`~`) to style the sibling based on the button's focus/hover states.
