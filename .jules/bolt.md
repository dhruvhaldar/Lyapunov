...
## 2026-05-18 - Caching Redundant Math Evaluations
**Learning:** In hot loops such as numerical integration or real-time control (like `FeedbackLinearization`), executing `math.sin(t)` multiple times within the same step introduces measurable function call and trigonometric evaluation overhead.
**Action:** When evaluating trigonometric functions or other expensive operations on variables that remain static within a given scope (like the time `t` inside a single `compute` step), always cache the result into a local variable (e.g., `sin_t = math.sin(t)`) and reuse it for subsequent calculations (like `r = sin_t`, `r_ddot = -sin_t`). This improves performance without sacrificing mathematical readability.
