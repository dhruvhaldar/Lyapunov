...
## 2026-05-19 - Three.js WebGL GPU Memory Leak
**Learning:** Removing a mesh from the Three.js scene (e.g., `scene.remove(mesh)`) does not automatically free up the underlying WebGL buffers associated with its geometry and material. If a long-running app repeatedly spawns and removes meshes without explicitly calling `.dispose()` on their resources, the browser will eventually crash due to OOM (Out of Memory) GPU errors.
**Action:** Always traverse a removed object and explicitly call `child.geometry.dispose()` and `child.material.dispose()` (or iterate over material arrays) to ensure WebGL resources are released when the object is no longer needed.
