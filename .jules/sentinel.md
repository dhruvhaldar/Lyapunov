...
## 2024-08-01 - XSS Vulnerability due to 'unsafe-inline' styles
**Vulnerability:** The application was vulnerable to Cross-Site Scripting (XSS) and injection risks because the `Content-Security-Policy` header in `api/index.py` allowed `'unsafe-inline'` for `style-src`. This could allow attackers to inject malicious styles that alter the UI or exfiltrate data (e.g. via CSS attribute selectors).
**Learning:** Allowing `'unsafe-inline'` in `style-src` is risky. While dynamically setting styles via CSSOM (`element.style.property = 'value'`) is safe under strict CSP, inline `style="..."` attributes in HTML and `.cssText` assignments are blocked.
**Prevention:** Strictly forbid `'unsafe-inline'` in the `style-src` directive of the Content-Security-Policy (CSP) header. Extract all inline CSS into dedicated external files or define semantic classes instead of relying on inline styles in HTML and JS.
