...
## 2024-05-20 - DoS/Exception bypassing Pydantic limits via nested fields and floats
**Vulnerability:** Denial of Service (DoS) risk and unhandled exception risks. Although Pydantic models restricted top-level dictionary lengths or list sizes using `Field(..., max_length=X)`, the internal items of those collections (e.g. dictionary keys, string entries in a list) lacked constraints. Additionally, Python `float` accepts `inf` and `nan` values which bypass normal numeric processing and can crash numeric libraries like NumPy or SymPy.
**Learning:** Pydantic's `Field` validation applied to a `List` or `Dict` does not recursively validate the size or constraints of inner primitive types. `float` values in APIs must explicitly reject `inf`/`nan` unless the downstream logic specifically handles them safely.
**Prevention:** Always apply primitive constraints to nested data structures using Pydantic's constraint types like `constr(max_length=...)` for strings inside collections, and `confloat(allow_inf_nan=False)` for floats that feed into numerical logic.
