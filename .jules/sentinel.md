...
## 2024-07-28 - XSS/Injection Risk via unsafe-inline styles\n**Vulnerability:** The application was vulnerable to injection attacks because the `Content-Security-Policy` header in `api/index.py` allowed `unsafe-inline` for `style-src`.\n**Learning:** Allowing `unsafe-inline` in CSS directives defeats a primary purpose of Content Security Policy, potentially enabling style injection attacks.\n**Prevention:** Strictly forbid `unsafe-inline` in both `script-src` and `style-src` directives. Extract all inline CSS into external stylesheets.
