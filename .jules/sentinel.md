...
## 2024-07-27 - XSS Vulnerability via 'unsafe-inline' in style-src
**Vulnerability:** The application's Content Security Policy (CSP) allowed `'unsafe-inline'` in the `style-src` directive. This could allow an attacker to execute arbitrary CSS injection, potentially leading to data exfiltration or UI redressing if they can inject a `<style>` tag or `style` attribute into the DOM.
**Learning:** Allowing `'unsafe-inline'` for styles defeats a major part of CSP by leaving the application vulnerable to CSS-based attacks. Furthermore, removing `'unsafe-inline'` will break all inline `style="..."` attributes in HTML, requiring them to be refactored into CSS classes.
**Prevention:** To prevent CSS injection and XSS risks, explicitly remove `'unsafe-inline'` from the `style-src` directive in the backend CSP configuration. Extract all inline CSS into dedicated external files and apply styles using semantic classes. Note that setting styles programmatically via the CSSOM API (e.g., `element.style.color = 'red'`) remains safe and allowed under strict CSP.
