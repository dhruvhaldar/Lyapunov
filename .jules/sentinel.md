...
## 2024-05-20 - DoS via JSON Serialization Crash on inf/nan
**Vulnerability:** Denial of Service (DoS). By default, Pydantic's `float` allows parsing "NaN", "Infinity", and "-Infinity". When these invalid mathematical inputs bypass validation and are passed back to the user within a 422 `RequestValidationError` response, FastAPI's default JSON serializer crashes with a 500 error (`ValueError: Out of range float values are not JSON compliant: nan`), taking down the worker thread.
**Learning:** Returning invalid data in the default Pydantic/FastAPI exception response is a risk vector if the data causes the JSON encoder itself to crash before the response goes out.
**Prevention:** Use `confloat(allow_inf_nan=False)` in Pydantic models for all float fields. Additionally, implement a custom exception handler for `RequestValidationError` that recursively sanitizes any `nan` or `inf` values inside the error details or request body (converting them to strings) before constructing the `JSONResponse`.
