...
## 2026-06-17 - Adding hashchange listener for SPA navigation
**Learning:** For single-page applications without complex routing, syncing primary view configuration state to the URL hash using `window.history.pushState()` or `replaceState()` is excellent for bookmarkability. However, without a `hashchange` event listener on the `window`, the UI will not update automatically when users navigate via browser back/forward buttons or manually edit the URL, leading to a broken state.
**Action:** Always implement a `hashchange` event listener on the `window` to read the new hash and update the UI accordingly. Ensure you use `pushState` only when the hash hasn't already been updated by the browser (to prevent duplicate history entries on back navigation).

## 2026-06-21 - Data Exploration Tactile Feedback
**Learning:** When users interact with dense data visualizations like D3 vector fields or Canvas time charts, standard pointer cursors fail to communicate the expected precision for selecting or inspecting data points. Furthermore, hovering over dense vector fields without explicit tactile feedback (e.g., highlighting the specific vector line) causes users to easily lose track of the line they are focused on.
**Action:** For interactive data visualizations containing dense data fields, always apply a `cursor: crosshair` to the container to clearly signal precision data selection capabilities. Additionally, implement tactile visual feedback on hover (e.g., stroke widening and color changes, wrapped in `@media (hover: hover)`) to help users accurately track specific elements within the dense field.

## 2026-06-22 - Skip Link Hash Collision in SPAs
**Learning:** When implementing "Skip to content" links using native anchor tags (e.g., `<a href="#main-content">`), the browser natively mutates the URL hash upon interaction. For Single Page Applications (SPAs) that already rely on the URL hash for primary state routing (like `#VanDerPol` to load a specific system model), this native behavior causes a destructive collision. Using the accessibility skip link actively overwrites the SPA routing hash, immediately breaking the user's ability to bookmark or share their current configured view.
**Action:** When implementing skip links in SPAs that utilize hash-based routing, intercept the link's `click` event with JavaScript. Call `e.preventDefault()` to stop the native URL hash mutation, and instead manually manage the focus transfer by programmatically calling `.focus()` on the target container. This preserves the routing state while maintaining keyboard accessibility.

## 2026-06-26 - Dismissing Focus with Escape Key
**Learning:** When using tactile visual feedback or persistent focus indicators (like keyboard shortcut hints that remain visible while an element has focus), keyboard users can become "trapped" in a focused state if they simply want to dismiss the UI change without activating the element or tabbing entirely out of the region. This is especially true in interfaces heavily utilizing global keyboard shortcuts, where a user might accidentally tab into an element and want to clear their state before typing a global command.
**Action:** Always provide an explicit "Escape" hatch for keyboard users. Listen for the `Escape` key globally and call `document.activeElement.blur()` (as long as the active element isn't the body or a complex widget that needs `Escape` for its own inner state) to allow users to easily exit focused states and return to a neutral page state.

## 2026-06-27 - Preventing Escape Key Hijacking on Form Controls
**Learning:** When implementing a global "Escape" hatch to dismiss focus from custom interactive elements, blindly calling `document.activeElement.blur()` on all elements breaks native browser behavior for standard form controls. For example, if a user opens a `<select>` dropdown and presses Escape to close the dropdown menu, the global listener will violently blur the `<select>` element itself, completely removing the user's keyboard focus from the control and forcing them to tab back into it.
**Action:** Always explicitly exclude native form controls (like `INPUT`, `TEXTAREA`, and `SELECT`) from global programmatic `blur()` logic to ensure native sub-interactions (like closing dropdowns or clearing input text) function correctly without breaking the user's keyboard navigation flow.
...
## 2026-08-10 - Explicit Visual Feedback for Halting Animations via Reduced Motion
**Learning:** While honoring `prefers-reduced-motion` in WebGL animations prevents vestibular triggering, silently halting the animation makes the UI appear broken or frozen to sighted users who have the OS setting enabled. This causes confusion since standard web animations simply disappear, but a stationary WebGL canvas retains its visual presence.
**Action:** Always provide an explicit visual indicator (like a "Reduced Motion" badge) explaining *why* an otherwise interactive UI component (like a 3D visualization) is static. This reassures users that the feature is functioning as intended and adapting to their accessibility preferences.
