...
## 2026-04-19 - WCAG 2.2.2 Pause, Stop, Hide and Keyboard Focus
**Learning:** Adding a `focus` event listener to implement "Pause, Stop, Hide" on a non-focusable container (`<div>` or `<canvas>`) doesn't work out of the box because those elements aren't inherently in the tab order. To truly make the interaction accessible to keyboard-only users, the container itself must explicitly have `tabindex="0"` set in HTML or via JavaScript.
**Action:** When implementing interaction handlers (like `focus` and `blur`) for custom container elements (like WebGL canvases or interactive SVGs), always verify or assign `tabindex="0"` to the container so that it can receive programmatic and keyboard focus.
