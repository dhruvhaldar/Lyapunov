...
## 2026-04-18 - Pause WebGL/Canvas Animations and Dynamic Document Titles
**Learning:** For continuous `<canvas>` animations (like Three.js), users who are sensitive to motion or rely on screen readers need the ability to manually pause the animation (WCAG 2.2.2 "Pause, Stop, Hide"), even if they haven't set `prefers-reduced-motion` at the OS level. Additionally, when a single-page application dynamically changes its core context (like swapping the entire mathematical model being analyzed), keeping `document.title` static means screen reader users and those with multiple tabs open lose context of the active state.
**Action:** When implementing continuous 3D or data-heavy animations, always provide an interactive toggle (like pausing on `mouseenter` and `focus`) to allow users to halt the motion. Furthermore, when syncing contextual UI labels, always remember to dynamically update `document.title` to reflect the active application state.
