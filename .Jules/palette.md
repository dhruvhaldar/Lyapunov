...
## 2026-05-26 - Keyboard Interaction on aria-disabled Elements
**Learning:** When using `aria-disabled="true"` to disable interactive elements (like `<select>` or `<button>`) without dropping focus, relying on `click` and `mousedown` `e.preventDefault()` handlers is insufficient. Screen readers and keyboard-only users can still interact with the element using `Space`, `Enter`, or `Arrow` keys.
**Action:** When implementing custom disable logic via `aria-disabled="true"`, always include a capturing `keydown` event listener that calls `e.preventDefault()` and `e.stopPropagation()` for interactive keys, while explicitly allowing `e.key === 'Tab'` to ensure users can navigate away from the disabled element without getting trapped.
