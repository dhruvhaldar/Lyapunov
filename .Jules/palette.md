...
## 2026-08-30 - Responsive SVG Initialization
**Learning:** When dynamically appending SVGs (e.g., via D3.js) to a responsive container, hardcoding fixed `width` and `height` attributes based on the container's initial size prevents scaling.
**Action:** Always use a `viewBox` attribute (e.g., `viewBox="0 0 ${width} ${height}"`), `preserveAspectRatio="xMidYMid meet"`, and set CSS `width` and `height` to `100%` to ensure the visualization seamlessly scales on window or device resize.
